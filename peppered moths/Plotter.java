...
{
    private static final Color LIGHT_GREEN = new Color(0, 200, 0, 40);

    private static JFrame frame;
    private static GraphPanel graph;
    private static JLabel stepLabel;
    private static JLabel pepperedLabel;
    ...
    private void updateGraph()
    {
        ...
        for(int i = 0; i < class1Objects.size(); i++)
        {
            Moth moth = (Moth) class1Objects.get(i);
            if(moth.isPeppered()) peppered++;
            else black++;
        }
        graph.update(step, class2Objects.size(), class2Objects.size());
    ...
    private JFrame makeFrame(int width, int height, int startMax)
    {
        ...
        JPanel bottom = new JPanel();
            ...
            bottom.add(new JLabel("  Peppered:"));
            pepperedLabel = new JLabel("");
            bottom.add(pepperedLabel);
            
            bottom.add(new JLabel("  Black:"));
            blackLabel = new JLabel("");
            bottom.add(blackLabel);
    ...
    class GraphPanel extends JComponent
    {
        ...
        public void update(int step,int count1, int count2)
        {
            ...
            int y = height - ((height * peppered) / yMax) - 1;
            if (y<0) {
                scaleDown();
                y = height - ((height * peppered) / yMax) - 1;
            }
            ...
            g.drawLine(width-3, lastVal3+1, width-2, y);
            lastVal3 = y;
            
            repaint();
            
            stepLabel.setText("" + step);
            pepperedLabel.setText("" + peppered);
